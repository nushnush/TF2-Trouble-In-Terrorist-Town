...
stock void PerformThunder(const TTTPlayer player, int target)
{
	...
	if (player.killCount == 1)
	{
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			if (IsValidClient(i) && IsPlayerAlive(i) && i != client)
			{
				...
				int wep = TTTPlayer(i).SpawnWeapon("tf_weapon_sniperrifle", 851, 1, 6, "308 ; 1.0 ; 297 ; 1.0 ; 305 ; 1.0");
				SetAmmo(i, wep, 1);
			}
		}
	}
...
public Action Freeze(Handle hTimer, int client)
{
